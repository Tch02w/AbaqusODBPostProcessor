...
13. “取消当前组”只停止当前运行组，并在取消完成后继续下一个排队组，不结束
    `standard.exe` 求解。退出时若仍有运行/排队任务，可选择返回，或取消全部并退出；
    程序等待其启动的 Abaqus 子进程结束后才退出。
...
“加载方向”显示为 `X方向、Z方向、XZ方向、自动`，内部对应 Abaqus 的
`1、3、1+3`；自动模式继续按 ODB 文件名中的 U/V 工况判断。
