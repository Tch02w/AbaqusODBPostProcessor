...
def scan_field_ranges(abaqus_command: str, job_config_path: Path, output_path: Path,
    ...
    """Use Abaqus' active-display legend extrema; only damage detection reads values."""
    output_path.parent.mkdir(parents=True, exist_ok=True)
    run_process([abaqus_command, "cae", f"noGUI={abaqus_script('scan_legend_limits_compat_v2.py')}", "--",
...
def run_job(abaqus_command: str, job_config_path: Path, log: LogCallback | None = None) -> None:
    run_process([abaqus_command, "cae", f"noGUI={abaqus_script('extract_job_compat_v7.py')}",
