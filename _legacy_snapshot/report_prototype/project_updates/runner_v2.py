...
def scan_field_ranges(
    ...
    run_process(
        [abaqus_command, "python", str(abaqus_script("scan_field_ranges.py")), "--config", str(job_config_path), "--output", str(output_path)],
...
def run_job(
    ...
    run_process(
        [abaqus_command, "cae", f"noGUI={abaqus_script('extract_job_compat_v5.py')}", "--", str(job_config_path)],
        cwd=job_config_path.parent,
        log=log,
    )
