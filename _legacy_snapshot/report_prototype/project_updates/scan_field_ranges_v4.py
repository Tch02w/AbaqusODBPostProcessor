"""Add correct pile-node U scanning to the field-range worker."""
...
if not this_path:
    import sys
    candidates = [value for value in sys.argv if value.lower().replace("/", "\\").endswith("scan_field_ranges_v4.py")]
    if not candidates: raise RuntimeError("Cannot locate scan_field_ranges_v4.py in argv")
...
new = (
    '                try:\n'
    '                    values = (frame.fieldOutputs[field_name].values if field_name == "U" else\n'
    '                              frame.fieldOutputs[field_name].getSubset(region=regions[region_name]).values)\n'
    '                except Exception: continue\n'
    '                selected = []\n'
    '                for value in values:\n'
    '                    owner = getattr(value.instance, "name", "")\n'
    '                    if field_name == "U":\n'
    '                        key = (owner, int(getattr(value, "nodeLabel", -1)))\n'
    '                        if key not in pile_node_keys: continue\n'
    '                    else:\n'
    '                        key = (owner, int(getattr(value, "elementLabel", -1)))\n'
    '                    if region_name == "rebar" and key not in rebar_keys: continue\n'
)
source = source.replace(old, new)
for marker in ("pile_node_keys = set()", 'values if field_name == "U"', "key not in pile_node_keys"):
