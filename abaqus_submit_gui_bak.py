...
def save_config():
    ...
    config = {
        "last_inp_file": "",
        "last_oldjob_file": oldjob_var.get().strip(),
        "last_for_file": for_file_var.get().strip(),
...
def append_submit_history(job_state):
    ...
    memory_text = job_state["memory_argument"] if job_state["memory_argument"] else "默认内存"
    ...
    append_history_text(
        f"[{submit_time}] 已提交：{job_name} | 核心数：{cpus_text} | "
        f"{memory_text} | {datacheck_text} | {option_text} | "
        f"{output_mode} | {overwrite_text}\n"
        f"命令：{job_state['cmd']}\n"
    )
...
def append_job_final_history(job_state, status, detail=""):
    ...
    append_history_text(
        f"[{finish_time}] {job_state['job_name']} 结束状态：{status}"
...
def inspect_job_files(work_dir, job_name, submitted_after=0):
    ...
    for extension in DIAGNOSTIC_EXTENSIONS:
        ...
        if submitted_after and os.path.exists(path):
            try:
...
def maybe_answer_overwrite_prompt(process, job_state, output_buffer):
    ...
    if job_state is None or job_state.get("overwrite_answer_sent"):
        return
...
def apply_saved_config():
    """把保存的路径显示到选择框中。"""
    last_inp_file = saved_config.get("last_inp_file", "")
    last_oldjob_file = saved_config.get("last_oldjob_file", "")
    last_for_file = saved_config.get("last_for_file", "")

    if last_inp_file:
        inp_file_var.set(last_inp_file)
        job_name_var.set(os.path.splitext(os.path.basename(last_inp_file))[0])
    ...
    if last_oldjob_file:
        oldjob_var.set(last_oldjob_file)
        set_optional_file_entry(
            oldjob_entry,
            last_oldjob_file,
            OLDJOB_PLACEHOLDER,
    ...
    if last_for_file:
        for_file_var.set(last_for_file)
        set_optional_file_entry(
            for_file_entry,
            last_for_file,
...
def create_job_log_tab(job_state):
    ...
    log_tab_counter += 1
    tab_title = f"{job_name} #{log_tab_counter}"
...
def finalize_job(job_state, status, detail=""):
    ...
    set_job_status(job_state, f"{status} | {detail}" if detail else status)
...
def start_process_output_monitor(process, log_widget, job_state=None):
    ...
    def read_output():
        ...
        # 处理最后一段没有换行的输出
        if line_buffer and not should_hide_console_line(line_buffer):
            if job_state is not None:
...
def monitor_datacheck_files(monitor_state):
    ...
    if any_file_seen:
        set_job_status(job_state, "Datacheck | 检查中")
    else:
        set_job_status(job_state, "Datacheck | 等待诊断文件")
...
def monitor_sta_file(monitor_state):
    ...
    if not os.path.exists(sta_path):
        ...
        if process.poll() is not None:
            ...
            if final_status:
                ...
                finalize_job(job_state, "失败", f"Abaqus 进程异常结束，返回码 {process.returncode}")
            else:
                finalize_job(job_state, "结束", "Abaqus 进程结束，但未生成 .sta 文件")
            return

        # 重点：没有 .sta 不再作为中断/失败条件
        root.after(
            STA_POLL_INTERVAL_MS,
            lambda: monitor_sta_file(monitor_state)
        )
    ...
    if monitor_state["seen_sta"] and monitor_state["stable_no_lock_polls"] >= 3:
        ...
        if final_status:
            ...
            finalize_job(job_state, "结束", "lck文件已释放，但未检测到明确完成或错误信息")
        return
...
def set_optional_file_entry(entry, value, placeholder, prefix=""):
    ...
    if value:
        ...
        display_text = f"{prefix}：{filename}" if prefix else filename
        entry.insert(0, display_text)
        entry.configure(fg="#111827", justify="left")
    else:
...
def backup_existing_sta_file(work_dir, job_name, backup_odb_path=""):
    ...
    if backup_odb_path:
        ...
        backup_path = get_unique_backup_path(work_dir, job_name, ".sta")
...
def backup_existing_odb(odb_path):
    """将已有 ODB 重命名为 原名_bak_年月日小时分钟.odb。"""
    ...
    backup_path = get_unique_backup_path(work_dir, base_name, ".odb")
...
def submit_job():
    ...
    if existing_odb_file:
        ...
        if odb_action == "overwrite":
            ...
            try:
                backup_odb_path = backup_existing_odb(existing_odb_file)
            except OSError as e:
                messagebox.showerror(
                    "备份失败",
                    f"无法重命名原 ODB 文件：\n\n"
...
inp_file_entry.grid(row=0, column=0, columnspan=2, sticky="w", ipady=6, pady=(0, 18))
set_optional_file_entry(
    inp_file_entry,
    "",
    INP_FILE_PLACEHOLDER,
...
oldjob_entry.grid(row=1, column=0, columnspan=2, sticky="w", ipady=6, pady=(0, 18))
set_optional_file_entry(
    oldjob_entry,
    "",
    OLDJOB_PLACEHOLDER,
...
for_file_entry.grid(row=2, column=0, columnspan=2, sticky="w", ipady=6, pady=(0, 20))
set_optional_file_entry(
    for_file_entry,
    "",
    FOR_FILE_PLACEHOLDER,
...
interactive_check.grid(row=0, column=0, sticky="w")
...
datacheck_check.grid(row=0, column=1, sticky="w", padx=(18, 0))
...
notify_check.grid(row=0, column=2, sticky="w", padx=(18, 0))
