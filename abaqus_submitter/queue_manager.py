...
from .app_paths import CONFIG_PATH, JOBLIST_PATH
...
class QueueManagerDialog(QtWidgets.QDialog):
    ...
    def request_joblist_save(self) -> None:
        self.host_window.reconcile_scheduler_state()
    ...
    def build_ui(self) -> None:
        ...
        queue_toolbar.addWidget(self.release_queue_btn)
        queue_toolbar.addWidget(self.requeue_btn)
        ...
        self.release_queue_btn.clicked.connect(lambda: self.set_selected_pending_hold(False))
        self.requeue_btn.clicked.connect(self.requeue_selected_terminal)
    ...
    def update_table_cell(
        ...
        if table is self.queue_table and column in (
            FORMAL_CORE_COLUMN,
            FORMAL_MEMORY_COLUMN,
            FORMAL_PRIORITY_COLUMN,
        ):
    ...
    def apply_formal_item_edit_result(self, item: QueueItem) -> None:
        ...
        if item.run_mode == "restart" and not self.restart_dependency_resolved(item):
            ...
            item.message = ("用户暂停调度" if item.held else "待提交") if item.valid else item.message
    ...
    def on_queue_item_double_clicked(self, table_item: QtWidgets.QTableWidgetItem) -> None:
        ...
        if column == FORMAL_FORTRAN_COLUMN:
            ...
            if path:
                ...
                self.request_joblist_save()
            return
        if column in (FORMAL_CORE_COLUMN, FORMAL_MEMORY_COLUMN, FORMAL_PRIORITY_COLUMN):
    ...
    def on_queue_table_item_changed(self, table_item: QtWidgets.QTableWidgetItem) -> None:
        column = table_item.column()
        if column not in (FORMAL_CORE_COLUMN, FORMAL_MEMORY_COLUMN, FORMAL_PRIORITY_COLUMN):
            return
        row_key = self.table_row_key(self.queue_table, table_item.row())
        item = self.queue_item_by_row_key(row_key)
        ...
        if column == FORMAL_CORE_COLUMN:
            try:
                ...
                self.refresh_queue_table()
                return
        elif column == FORMAL_MEMORY_COLUMN:
            item.memory = value
        else:
            try:
                item.priority = max(-10000, min(10000, int(value)))
            except ValueError:
                self.refresh_queue_table()
                return
        self.apply_formal_item_edit_result(item)
    ...
    def cancel_selected_pending(self) -> None:
        ...
        pending_ids = {
            item.item_id
            for item in self.queue_items
            if item.item_id in selected_keys and self.queue_item_is_editable(item)
        }
        blocked = any(
            item.item_id in selected_keys
            and item.status in (STATUS_RUNNING, STATUS_STARTING, STATUS_TERMINATING)
            for item in self.queue_items
        )
        if pending_ids:
    ...
    def set_selected_pending_hold(self, held: bool) -> None:
        ...
        if not item_ids:
            QtWidgets.QMessageBox.information(self, "修改调度状态", "请选择待运行或等待前置的作业。")
            return
        self.host_window.set_queue_items_hold(item_ids, held)

    def requeue_selected_terminal(self) -> None:
        selected_keys = self.selected_table_row_keys(self.queue_table)
        ...
        item_ids = {
            item.item_id
            for item in self.queue_items
            if item.item_id in selected_keys and item.status in requeueable_statuses
        }
        if not item_ids:
            QtWidgets.QMessageBox.information(self, "重新排队", "请选择失败、取消或状态丢失的作业。")
            return
        self.host_window.requeue_terminal_queue_items(item_ids)
    ...
    def edit_selected_pending(self) -> None:
        ...
        item.priority = priority_spin.value()
    ...
    def resize_formal_columns(self) -> None:
        widths = (40, 140, 260, 90, 110, 110, 60, 70, 64, 72, 110)
