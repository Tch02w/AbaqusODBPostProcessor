...
def sync_tracking_state_from_legacy_job_state(
    ...
    tracking_state.monitor_mode = str(
        legacy_state.get("memory_monitor_mode", tracking_state.monitor_mode) or "learning"
    )
    tracking_state.next_sample_at = float(legacy_state.get("next_memory_sample_at", tracking_state.next_sample_at) or 0)
    tracking_state.memory_samples = [
        int(value or 0) for value in legacy_state.get("memory_samples", tracking_state.memory_samples) or []
    ]
