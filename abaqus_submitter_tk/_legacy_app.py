...
from .models import MemoryStatusEx, QueueItem
from .memory_monitor import (
    JobMemoryEstimate,
    JobMemoryUsage,
    MemoryMonitorService,
    get_job_memory_step as service_get_job_memory_step,
    infer_model_group as service_infer_model_group,
...
def get_memory_safety_factor():
    ...
    value = float(memory_safety_factor_state.get("value", JOB_MEMORY_BASE_SAFETY_FACTOR))
    memory_monitor_service.safety_factor = value
    return value
...
def increase_memory_safety_factor(reason=""):
    ...
    memory_monitor_service.safety_factor = updated
    append_history_text(
...
def infer_model_group(job_name):
    ...
    return service_infer_model_group(job_name)


def get_job_memory_step(job_state):
    """Return current Abaqus step for step-level memory peaks."""
    return service_get_job_memory_step(job_state)


def memory_estimate_to_legacy_dict(estimate):
    """Convert a service estimate into the legacy dictionary shape."""
    if isinstance(estimate, JobMemoryEstimate):
        return {
            "group": estimate.group,
            "estimated_memory": estimate.estimated_memory,
            "peak_memory": estimate.peak_memory,
            "sample_count": estimate.sample_count,
            "process_count": estimate.process_count,
            "process_names": estimate.process_names,
            "updated_at": estimate.updated_at,
            "stable": estimate.stable,
            "step_peaks": dict(estimate.step_peaks),
        }
    return dict(estimate or {})


def sync_legacy_memory_estimate(job_name):
    """Keep legacy job_memory_estimates compatible with older UI code."""
    estimate = memory_monitor_service.job_estimates.get(job_name)
    if estimate is not None:
        job_memory_estimates[job_name] = memory_estimate_to_legacy_dict(estimate)
...
def estimate_per_job_memory_from_running_jobs():
    ...
    per_job_memory, sample_count = memory_monitor_service.estimate_per_job_memory(
        usage_by_job=usage_by_job,
        active_job_names=active_job_names,
    )
    return per_job_memory, sample_count, usage_by_job
...
def update_job_memory_sample(job_state, usage):
    ...
    get_memory_safety_factor()
    job_key = job_state.get("job_key", "")
    job_name = job_state.get("job_name", "")
    if not job_key or not job_name:
        return

    tracking_state = memory_monitor_service.register_job(
        job_key=job_key,
        job_name=job_name,
        work_dir=job_state.get("work_dir", ""),
        monitor_mode=job_state.get("memory_monitor_mode", "learning"),
    )
    sync_tracking_state_from_legacy_job_state(tracking_state, job_state)
    monitor_mode_before = tracking_state.monitor_mode
    events = memory_monitor_service.apply_usage_snapshot(
        due_job_keys=[job_key],
        usage_by_job={job_name: usage},
        step_by_job_key={job_key: get_job_memory_step(job_state)},
        now=time.monotonic(),
    )
    sync_legacy_job_state_from_tracking_state(job_state, tracking_state)
    if monitor_mode_before == "patrol" and tracking_state.monitor_mode == "learning":
        job_state["memory_stable_logged"] = False
    sync_legacy_memory_estimate(job_name)

    for event in events:
        if event.get("event_type") != "memory_estimate_stable":
            continue
        if job_state.get("memory_stable_logged"):
            continue
        ...
        append_history_text(
            f"{job_name}????????????"
            f"?? {format_memory_size(event.get('peak_memory', 0))}?"
            f"?? {format_memory_size(event.get('estimated_memory', 0))}?"
            "???????\n\n"
        )
...
def start_job_memory_monitor(job_state):
    ...
    job_state["memory_monitor_active"] = False
    tracking_state = memory_monitor_service.register_job(
        job_key=job_state.get("job_key", ""),
        job_name=job_state.get("job_name", ""),
        work_dir=job_state.get("work_dir", ""),
...
def update_external_job_memory_estimate(item):
    ...
    get_memory_safety_factor()
    estimate = memory_monitor_service.update_external_job_estimate(
    ...
    if estimate is not None:
        job_memory_estimates[item.job_name] = memory_estimate_to_legacy_dict(estimate)
...
def activate_job_memory_monitor(job_state):
    ...
    sync_tracking_state_from_legacy_job_state(tracking_state, job_state)
    memory_monitor_service.activate_job(job_state.get("job_key", ""), now=time.monotonic())
    sync_legacy_job_state_from_tracking_state(job_state, tracking_state)
...
def estimate_available_job_slots():
    """Estimate available queue slots from measured memory samples."""
    _, available_memory = get_system_memory_info()
    _, _, job_memory_usage = estimate_per_job_memory_from_running_jobs()
    active_job_names = {
        state.get("job_name", "")
        for state in active_jobs.values()
        if not state.get("finalized")
    }
    slot_estimate = memory_monitor_service.estimate_available_slots(
        available_memory=available_memory,
        usage_by_job=job_memory_usage,
        active_job_names=active_job_names,
    )
    return slot_estimate.slots, {
        "memory_slots": slot_estimate.memory_slots,
        "memory_limited": slot_estimate.memory_limited,
        "available_memory": slot_estimate.available_memory,
        "usable_memory": slot_estimate.usable_memory,
        "current_abaqus_memory": slot_estimate.current_abaqus_memory,
        "memory_available_for_new_jobs": slot_estimate.memory_available_for_new_jobs,
        "per_job_memory": slot_estimate.per_job_memory,
        "process_per_job_memory": slot_estimate.per_job_memory,
        "memory_sample_count": slot_estimate.memory_sample_count,
...
def finalize_job(job_state, status, detail=""):
    ...
    if job_state.get("memory_peak"):
        ...
        sync_tracking_state_from_legacy_job_state(tracking_state, job_state)
        memory_monitor_service.finalize_job(job_state.get("job_key", ""))
        sync_legacy_job_state_from_tracking_state(job_state, tracking_state)
        sync_legacy_memory_estimate(job_state["job_name"])
