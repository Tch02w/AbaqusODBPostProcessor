...
candidates = [value for value in sys.argv if value.lower().replace("/", "\\").endswith("extract_job_visualization.py")]
if not candidates: raise RuntimeError("Cannot locate extract_job_visualization.py in argv")
script_path = os.path.abspath(candidates[-1]); directory = os.path.dirname(script_path)
base_path = os.path.join(directory, "extract_job_pipeline.py")
with open(base_path, "r", encoding="utf-8") as stream: source = stream.read()
source = source.replace("extract_job_pipeline.py", "extract_job_visualization.py")
