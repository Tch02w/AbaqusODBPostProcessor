...
class JobController:
    ...
    @property
    def runtime_controller(self) -> LocalExecutionBackend:
    ...
    def scheduler(self) -> SchedulerCore:
        return self.window.scheduler

    @property
    def memory_monitor_service(self) -> Any:
        return self.window.memory_monitor_service

    @property
    def latest_system_memory(self) -> dict[str, int]:
        return self.window.latest_system_memory
    ...
    def _estimated_memory_by_job(self) -> dict[str, int]:
        return {
            str(job_name): int(estimate.estimated_memory or estimate.peak_memory or 0)
            for job_name, estimate in self.memory_monitor_service.job_estimates.items()
        }

    def _reconcile_scheduler(self, queue_items: list[QueueItem]) -> None:
        reconcile_scheduler_from_queue(
            self.scheduler,
            queue_items,
            estimated_memory_by_job=self._estimated_memory_by_job(),
        )

    def _scheduler_resource_snapshot(
        self,
        slot_info: DispatchSlotInfo,
        queue_items: list[QueueItem],
    ) -> ResourceSnapshot:
        available_memory = int(self.latest_system_memory.get("available") or 0)
        active_statuses = scheduler_managed_active_statuses()
        active_cores = sum(
            max(0, int(item.cores or 0))
            for item in queue_items
            if item.status in active_statuses
        )
        return ResourceSnapshot(
            available_slots=self._dispatch_available_slots(slot_info, queue_items),
            available_cores=max(0, MAX_CPUS - active_cores),
            available_memory_bytes=available_memory if available_memory > 0 else None,
        )
    ...
    def _apply_schedule_plan(self, queue_items: list[QueueItem], plan) -> None:  # noqa: ANN001
        items_by_job_id = {item.job_id: item for item in queue_items}
        for snapshot in plan.jobs:
            item = items_by_job_id.get(snapshot.job_id)
            if item is not None:
                apply_scheduler_snapshot_to_queue_item(snapshot, item)

    def _start_scheduled_allocation(
        self,
        item: QueueItem,
        options: SubmitOptions,
        *,
        ...
        self._apply_scheduler_event(item, ExecutionEventKind.STARTING, "正在准备本机执行环境")
        started = self.start_job(options, queue_item=item, queue_mode=queue_mode)
        if started:
            return True

        self._record_scheduled_start_failure(item)
        return False

    def _record_scheduled_start_failure(self, item: QueueItem) -> None:
        if item.status in {STATUS_PENDING_RUN, STATUS_WAITING_DEPENDENCY, STATUS_STARTING}:
            snapshot = self.scheduler.requeue(item.job_id, item.message or "启动前条件尚未满足")
            apply_scheduler_snapshot_to_queue_item(snapshot, item, update_message=False)
            return

        event_kind = (
            ExecutionEventKind.CANCELED
            if item.status in {STATUS_CANCELED, STATUS_TERMINATED}
            else ExecutionEventKind.FAILED
        )
        self._apply_scheduler_event(item, event_kind, item.message or "启动失败")
    ...
    def submit_scheduled_job(
        ...
        *,
        ...
        item.attempt_id = ""
        item.message = "等待本机调度"
        self._reconcile_scheduler(self.queue_items)
        slot_info = self._dispatch_slot_info()
        plan = self.scheduler.plan(
            self._scheduler_resource_snapshot(slot_info, self.queue_items),
            archive_reserved_keys=self._archive_reserved_keys(),
            eligible_job_ids={item.job_id},
        )
        self._apply_schedule_plan(self.queue_items, plan)
        allocation = next(
            (allocation for allocation in plan.allocations if allocation.job_id == item.job_id),
            None,
        )
        if allocation is None:
            ...
            return False
        return self._start_scheduled_allocation(item, options, queue_mode=queue_mode)
    ...
    def stop_queue(
        ...
        for item in self.queue_items:
            if item.status in {STATUS_PENDING_RUN, STATUS_WAITING_DEPENDENCY}:
                ...
                item.message = "用户终止队列"
                if self.scheduler.get(item.job_id) is not None:
                    self._apply_scheduler_event(item, ExecutionEventKind.CANCELED, item.message)
    ...
    def dispatch_queue_now(self) -> None:
        ...
        self._reconcile_scheduler(queue_items)
        slot_info = self._dispatch_slot_info()
        ...
        for item in self._pending_dispatch_items(queue_items):
            ...
            if not ok:
                ...
                continue
            eligible_job_ids.add(item.job_id)

        self._reconcile_scheduler(queue_items)
        plan = self.scheduler.plan(
            self._scheduler_resource_snapshot(slot_info, queue_items),
            archive_reserved_keys=archive_reserved_keys,
            eligible_job_ids=eligible_job_ids,
        )
        self._apply_schedule_plan(queue_items, plan)
        items_by_job_id = {item.job_id: item for item in queue_items}
        for allocation in plan.allocations:
            item = items_by_job_id.get(allocation.job_id)
            if item is None:
                continue
            options = queue_item_to_options(item, default_cpus=self._default_cpus())
            self._start_scheduled_allocation(item, options, queue_mode=True)
    ...
    def on_workspace_prepare_succeeded(self, task: WorkspacePrepareTask, result) -> None:
        ...
        if not started:
            self._record_scheduled_start_failure(queue_item)
            self.refresh_visible_queue_manager()
            self.update_queue_status_label()
            self.request_dispatch_queue()
    ...
    def on_workspace_prepare_failed(self, task: WorkspacePrepareTask, message: str, copied_inp_path: str = "") -> None:
        ...
        if self.scheduler.get(queue_item.job_id) is not None:
            self._apply_scheduler_event(queue_item, ExecutionEventKind.FAILED, queue_item.message)
        self.append_history(f"准备计算工作目录失败：{options.job_name}\n{message}")
    ...
    def continue_start_job_after_workspace_ready(
        ...
        *,
        ...
        if queue_item is not None:
            queue_item.status = STATUS_STARTING
            queue_item.message = "等待 Abaqus launcher 启动"
    ...
    def finalize_completed_run(self, job_key: str) -> None:
        ...
        if queue_item is not None:
            ...
            queue_item.active_job_key = ""
            terminal_event = ExecutionEventKind.LOST
            if decision.status in {STATUS_COMPLETED, STATUS_DATACHECK_COMPLETED}:
                terminal_event = ExecutionEventKind.SUCCEEDED
            elif decision.status in {STATUS_CANCELED, STATUS_TERMINATED}:
                terminal_event = ExecutionEventKind.CANCELED
            elif decision.status in {STATUS_FAILED, STATUS_DATACHECK_FAILED}:
                terminal_event = ExecutionEventKind.FAILED
            if self.scheduler.get(queue_item.job_id) is None:
                self._reconcile_scheduler(self.queue_items)
            self._apply_scheduler_event(queue_item, terminal_event, decision.message)
            self._refresh_queue_dependencies()
            self._refresh_queue_views()
