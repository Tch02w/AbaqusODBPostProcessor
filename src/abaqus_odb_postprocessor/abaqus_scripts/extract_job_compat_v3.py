...
script_candidates = [
    ...
    if value.lower().replace("/", "\\").endswith("extract_job_compat_v3.py")
]
...
if not script_candidates or not config_candidates:
    raise RuntimeError(
        "Cannot locate noGUI script and job JSON in argv: {0}".format(repr(sys.argv))
    )
...
if not os.path.isfile(source_path):
    raise RuntimeError(
        "Cannot locate extract_job.py beside noGUI script: script={0}, cwd={1}".format(
            script_argument, os.getcwd()
        )
    )
