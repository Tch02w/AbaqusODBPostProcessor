"""Final Abaqus worker: frame selection, free edges, orthographic high-resolution output."""
...
candidates = [value for value in sys.argv if value.lower().replace("/", "\\").endswith("extract_job_final_v10.py")]
if not candidates: raise RuntimeError("Cannot locate extract_job_final_v10.py in argv")
...
loader = loader.replace("extract_job_compat_v5.py", "extract_job_final_v10.py")
