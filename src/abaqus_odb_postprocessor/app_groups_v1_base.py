...
from . import app_groups_v1_legacy as _base
    ...
    def _run_scope(self, scope: str) -> None:
        ...
        def task(log: Callable[[str], None]) -> list[str]:
            ...
            def prepare_one(path: str) -> tuple[str, dict[str, Any]]:
                check_cancelled()
                scan_dir = run_root / "_scan" / _base.safe_folder_name(Path(path).stem)
            ...
            def extract_one(path: str) -> list[str]:
                ...
                for membership_index, group in enumerate(memberships):
                    check_cancelled()
                    if group["standalone"]:
                        output_dir = run_root / "standalone" / _base.safe_folder_name(Path(path).stem)
                    else:
                        output_dir = (
                            run_root
                            / "groups"
